...
def extract_elevation_data(tiff_file_path, output_file_path):
    ...
    try:
        ...
        with rasterio.open(tiff_file_path) as dataset:
            ...
            # For large files, we need to be smart about frontend formats
            # Frontend can't handle 21+ million points effectively
            
            # Method 1: Reasonable sample for frontend (max 100K points)
            max_frontend_points = 100000
            if total_points > max_frontend_points:
                sample_rate = total_points // max_frontend_points
                print(f"File too large for frontend - sampling every {sample_rate} points")
            else:
                sample_rate = 1
                print("File size OK for frontend - using all points")
            
            # Sample the data (now in WGS84)
            sample_indices = np.arange(0, total_points, sample_rate)
            lon_sampled = lon_rounded[sample_indices]  # longitude
            lat_sampled = lat_rounded[sample_indices]  # latitude
            z_sampled = z_rounded[sample_indices]      # elevation
            
            sampled_count = len(lon_sampled)
            print(f"Frontend data points: {sampled_count:,}")
            
            # Method 1: CSV for frontend (manageable size) - now in WGS84
            base_path = Path(output_file_path)
            csv_path = base_path.with_suffix('.csv')
            
            df = pd.DataFrame({
                'longitude': lon_sampled,  # WGS84 longitude
                'latitude': lat_sampled,   # WGS84 latitude
                'elevation': z_sampled     # elevation in meters
            })
            
            df.to_csv(csv_path, index=False, float_format='%.6f')
            csv_size_mb = os.path.getsize(csv_path) / 1000000
            print(f"Saved frontend CSV (WGS84): {csv_path} ({csv_size_mb:.2f} MB, {sampled_count:,} points)")
            
            # Method 2: JSON with metadata (best for frontend) - now in WGS84
            ...
            data_dict = {
                'metadata': {
                    ...
                    'original_total_points': total_points,
                    'frontend_points': sampled_count,
                    'sample_rate': sample_rate,
                    'nodata_value': float(dataset.nodata) if dataset.nodata is not None else None,
                    'bounds': {
                        'min_longitude': float(lon_sampled.min()),
                        'max_longitude': float(lon_sampled.max()),
                        'min_latitude': float(lat_sampled.min()),
                        'max_latitude': float(lat_sampled.max()),
                        'min_elevation': float(z_sampled.min()),
                        'max_elevation': float(z_sampled.max())
                    }
                },
                'data': {
                    'longitude': lon_sampled.tolist(),
                    'latitude': lat_sampled.tolist(),
                    'elevation': z_sampled.tolist()
                }
            }
            ...
            json_size_mb = os.path.getsize(json_path) / 1000000
            print(f"Saved frontend JSON (WGS84): {json_path} ({json_size_mb:.2f} MB)")
            
            # Method 3: GeoJSON format (already in WGS84)
            geojson_wgs84_path = str(base_path).replace('.txt', '_wgs84.geojson')
            
            # Create GeoJSON features (limit to reasonable number for mapping)
            max_geojson_points = 10000
            if sampled_count > max_geojson_points:
                geo_sample_rate = sampled_count // max_geojson_points
                geo_indices = np.arange(0, sampled_count, geo_sample_rate)
            else:
            ...
            print(f"\nFrontend-friendly file sizes (all in WGS84):")
            print(f"  CSV: {csv_size_mb:.2f} MB ({sampled_count:,} points)")
            print(f"  JSON: {json_size_mb:.2f} MB (with metadata)")
            print(f"  GeoJSON: {geojson_wgs84_size_mb:.2f} MB ({len(features_wgs84):,} points)")
            print(f"  Sample rate used: 1 in {sample_rate} points")
            ...
            with open(instructions_path, 'w') as f:
                ...
                f.write(f"Frontend points: {sampled_count:,}\n")
                f.write(f"Sample rate: 1 in {sample_rate}\n")
                f.write(f"Original coordinate system: {dataset.crs}\n")
                f.write(f"Output coordinate system: EPSG:4326 (WGS84 lat/lon)\n\n")
                f.write("Available frontend-friendly formats (all in WGS84):\n")
                f.write(f"1. {csv_path.name} - CSV data ({csv_size_mb:.2f} MB) - longitude,latitude,elevation\n")
                f.write(f"2. {json_path.name} - JSON with metadata ({json_size_mb:.2f} MB)\n")
                f.write(f"3. {Path(geojson_wgs84_path).name} - GeoJSON for web mapping ({geojson_wgs84_size_mb:.2f} MB)\n\n")
                f.write("Recommended for frontend:\n")
                f.write("- Use CSV for simple data loading (longitude, latitude, elevation columns)\n")
                f.write("- Use JSON for full metadata and coordinate bounds\n")
                f.write("- Use GeoJSON for web mapping libraries (Google Maps, Leaflet, etc.)\n")
                f.write("- All formats use WGS84 coordinates (EPSG:4326)\n")
                f.write("- All formats are directly readable by JavaScript\n")
                f.write(f"- Data is sampled to keep file sizes manageable (<5MB)\n")
