...
def extract_elevation_data(tiff_file_path, output_file_path):
    ...
    try:
        ...
        with rasterio.open(tiff_file_path) as dataset:
            ...
            # Common NoData values: -3.4028234663852886e+38, -inf, very large negative numbers
            ...
            if valid_elevation_count == 0:
                print(f"⚠️  WARNING: All elevation values are NoData/infinity ({z_values[0]:.2e})")
                print(f"⚠️  Skipping GeoJSON creation - no valid elevation data found")
                print(f"⚠️  This typically indicates missing or invalid raster data")
                
                # Create a simple info file instead
                info_path = Path(output_file_path).with_suffix('.info.txt')
                with open(info_path, 'w') as f:
                    f.write(f"TIFF FILE ANALYSIS - NO VALID DATA\n")
                    f.write(f"=" * 50 + "\n\n")
                    f.write(f"Source file: {Path(tiff_file_path).name}\n")
                    f.write(f"Dimensions: {width} x {height}\n")
                    f.write(f"Total pixels: {total_points:,}\n")
                    f.write(f"Original CRS: {dataset.crs}\n")
                    f.write(f"NoData value: {dataset.nodata}\n")
                    f.write(f"All elevation values: {z_values[0]:.2e} (NoData/infinity)\n\n")
                    f.write(f"STATUS: NO GEOJSON FILE CREATED\n")
                    f.write(f"REASON: All pixels contain NoData/infinity values\n")
                    f.write(f"RECOMMENDATION: Check source TIFF file for valid elevation data\n")
                
                print(f"Created info file: {info_path}")
                return 0  # Return 0 points since no valid data
            
            # Filter to only valid elevation points
            print(f"🔄 Filtering to only valid elevation points ({valid_elevation_count:,} points)...")
            print(f"💾 This will reduce file size by {((total_points - valid_elevation_count) / total_points) * 100:.1f}%")
            
            # Only keep valid points
            valid_lon_coords = lon_coords[valid_elevation_mask]
            valid_lat_coords = lat_coords[valid_elevation_mask]
            valid_z_values = z_values[valid_elevation_mask]
            
            print(f"✅ Filtered from {total_points:,} to {len(valid_z_values):,} points")
            
            # Round coordinates to reduce precision and file size
            # For WGS84: use 6 decimal places for lat/lon (≈1 meter precision)
            lon_rounded = np.round(valid_lon_coords, 6)  # longitude
            lat_rounded = np.round(valid_lat_coords, 6)  # latitude
            z_rounded = np.round(valid_z_values, 2)     # elevation
            ...
            with open(instructions_path, 'w') as f:
                ...
                f.write(f"Original pixels: {total_points:,}\n")
                f.write(f"Valid elevation points: {valid_elevation_count:,}\n")
                f.write(f"Output points: {sampled_count:,}\n")
                f.write(f"Data reduction: {((total_points - valid_elevation_count) / total_points) * 100:.1f}% (NoData/infinity filtered out)\n")
                ...
                f.write("- Only valid elevation points are included (NoData/infinity values filtered out)\n")
                f.write("- Missing areas represent NoData/infinity regions from original TIFF\n")
                ...
                f.write(f"- Contains {sampled_count:,} valid elevation points only\n")
            
            return valid_elevation_count  # Return count of valid points only
...
def process_all_tiff_files(input_directory, output_directory):
    ...
    print(f"\nProcessing complete!")
    print(f"Total VALID data points extracted: {total_points}")
    print(f"Output files saved in: {output_directory}")
    print(f"Note: Only valid elevation points stored - NoData/infinity values filtered out")
...
if __name__ == "__main__":
    ...
    print(f"Found {len(tiff_files)} TIFF files")
    print("Note: Only VALID elevation points will be processed")
    print("Optimization strategy:")
    print("  - NoData/infinity values filtered out")
    print("  - Only valid elevation points stored")
    print("  - Dramatic file size reduction")
    print("  - Frontend shows gaps for NoData regions")
    print("  - Full resolution GeoJSON for valid data only")
